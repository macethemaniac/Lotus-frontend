...
- Route-coverage testing may prefer a real multi-venue route when the quote amount is large enough: at least about `$49.95` can start as a pair route, and at least `$500` can expand to the widest executable pair/tri/strict-all style route available. This uses only live executable backend candidates, never fake legs. Standard production route policy remains economics-first and does not force splits unless the backend quote engine finds a valid improving route.
