...
- `GET /markets/{marketId}/chart?outcomeId&timeframe=1H|6H|1D|1W|1M|ALL` returns Lotus-owned live midpoint observations accumulated from backend orderbook reads. It does not use Predexon or browser venue calls; older timeframes may return `historyStatus: "accumulating"` until Lotus has observed enough live points.
